...
class WMS_TCPDF_STATIC {
	...
	public static function set_mqr( $mqr ) {
		if ( ! defined( 'WMS_PHP_VERSION_ID' ) ) {
			$version = PHP_VERSION;
			define( 'WMS_PHP_VERSION_ID', ( ( $version[0] * 10000 ) + ( $version[2] * 100 ) + $version[4] ) );
		}
		if ( WMS_PHP_VERSION_ID < 50300 ) {
	...
	public static function getObjFilename( $type = 'tmp', $file_id = '' ) {
		return tempnam( WMS_K_PATH_CACHE, '__tcpdf_' . $file_id . '_' . $type . '_' . md5( WMS_TCPDF_STATIC::getRandomSeed() ) . '_' );
	}
	...
	public static function getAnnotOptFromJSProp( $prop, &$spot_colors, $rtl = false ) {
		...
		if ( isset( $prop['fillColor'] ) ) {
			if ( is_array( $prop['fillColor'] ) ) {
				...
				$opt['mk']['bg'] = WMS_TCPDF_COLORS::convertHTMLColorToDec( $prop['fillColor'], $spot_colors );
			}
		}
		if ( isset( $prop['strokeColor'] ) ) {
			if ( is_array( $prop['strokeColor'] ) ) {
				...
				$opt['mk']['bc'] = WMS_TCPDF_COLORS::convertHTMLColorToDec( $prop['strokeColor'], $spot_colors );
			}
		}
	...
	public static function getHyphenPatternsFromTEX( $file ) {
		...
		foreach ( $patterns_array as $val ) {
			if ( ! WMS_TCPDF_STATIC::empty_string( $val ) ) {
				$val = trim( $val );
				$val = str_replace( '\'', '\\\'', $val );
